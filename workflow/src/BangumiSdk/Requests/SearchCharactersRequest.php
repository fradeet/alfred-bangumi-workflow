...
class SearchCharactersRequest extends Request implements HasBody
{
    use HasJsonBody;
    ...
    {
        return '/v0/search/characters';
    }

    /** @return PagedSearchResult<Character> */
    public function createDtoFromResponse(Response $response): PagedSearchResult
    {
        $data = $response->json();

        if (array_is_list($data)) {
            throw new \UnexpectedValueException('The character search response must be an object.');
        }

        $items = $data['data'] ?? null;

        if (!is_array($items) || !array_is_list($items)) {
            throw new \UnexpectedValueException('Character search field "data" must be a list.');
        }
        ...
        foreach ($items as $item) {
            if (!is_array($item) || array_is_list($item)) {
                throw new \UnexpectedValueException('Each character search result must be an object.');
            }

            $characters[] = Character::fromArray($item);
        }
    ...
    private static function requiredInt(array $data, string $key): int
    {
        ...
        if (!is_int($value)) {
            throw new \UnexpectedValueException(sprintf('Character search field "%s" must be an integer.', $key));
        }
